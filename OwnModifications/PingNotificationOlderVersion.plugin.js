...
let [
    ...
    { filter: Webpack.Filters.byKeys("hasThread") }, // hasThreadElementModule  (    { filter: Webpack.Filters.byKeys("hasThread", "nitroAuthorBadgeContainer", "isSystemMessage") }, // hasThreadElementModule)
...
// edit hotfix original repo
// if (!RecentMentionsInbox) {
//     Webpack.waitForModule(Webpack.Filters.byStrings(".clearMentions(),", ".deleteRecentMention")).then(m => { RecentMentionsInbox = m; });
// }
...
let liveMessages = [];
// edit hotfix original repo
//console.log("PN:diag", Object.entries({ NotificationUtils, NotificationSoundModule, MessageConstructor, transitionTo, Dispatcher, MessageActions, hasThreadElementModule, Message, messageReferenceSelectors, PopoutModule, RecentMentionsInbox, trailingModule, DiscordProgressBar, constructMessageObj, ChannelConstructor, useStateFromStores, appSidePanelSelectors, MemberAreaAvatar }).filter(([,v]) => v == null).map(([k]) => k));
...
module.exports = class PingNotification {
    ...
    onMessageReceived(event, update) {
        ...
        if (this.settings.keywordOnlyMode) { // Check if the user has restricted notifications to only specific keyword matches
            if (notifyResult?.isKeywordMatch === true) { // Verify if the current message contains a successfully matched keyword
    ...
    async threadDeleteHandler(event) {
        ...
        this.sessionMessages.forEach(item => {
            if (event.channel.id === item.channel_id && item.dummy === true) {
                ...
                item.fullMessage = new MessageConstructor(messageToConstruct);
                changed = true;
            }
        });
        ...
        if (this.settings.enableKeywordNotifications && 
            ...
            if (this.settings.notificationKeywords) {
                const keywords = this.settings.notificationKeywords
                    ...
                    .filter(Boolean); // Efficient way to filter out empty strings
                ...
                // Edit code added
                if (hasKeywordMatch) {
                    // We found a match, we can store which one for the UI
                    // (Searching again inside the 'some' block is inefficient, so we just set it here)
                    const matched = keywords.find(k => searchTarget.toLowerCase().includes(k.toLowerCase()));
                    keywordMatch = matched || "Keyword Match";
                }
            }
...
//let renderMessage; del

// ---「edit modifed code」 ---
// Safer RenderMessage that catches internal Discord crashes
// New change added just paramters for this funtion for now
